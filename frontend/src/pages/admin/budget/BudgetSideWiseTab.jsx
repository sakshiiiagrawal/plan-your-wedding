...
function SideCard({ label, color, icon: Icon, iconBg, iconColor, count, total, formatCurrency }) {
  return (
...
function ExpenseList({ items, emptyText, headerBg, headerBorder, headerColor, amountColor, totalBg, totalColor, formatCurrency, isShared }) {
  return (
      ...
      {items.length > 0 ? (
        <div className="divide-y divide-gray-100">
          ...
          <div className={`p-3 ${totalBg} flex justify-between font-bold ${totalColor}`}>
            <span>Total</span>
            <span>{formatCurrency(items.reduce((s, e) => s + parseFloat(e.amount || 0), 0))}</span>
          </div>
        </div>
      ) : (
...
export default function BudgetSideWiseTab({ sideWiseExpenses, formatCurrency }) {
  return (
