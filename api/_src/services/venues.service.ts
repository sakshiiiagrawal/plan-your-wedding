...
async function extractVenueFinanceInput(
  ...
  const itemsWithId = payload.existingItemId && items.length > 0
...
export async function deleteRoom(id: string) {
  ...
  if (hasGuests) {
    throw new BadRequestError('Cannot delete a room that has guests assigned. Remove guest assignments first.');
  }
