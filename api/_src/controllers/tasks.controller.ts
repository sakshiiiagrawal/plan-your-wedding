...
export const getAll = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    res.json(await service.listTasks(getWeddingOwnerId(req), {
...
export const updateStatus = async (req: Request<IdParam>, res: Response, next: NextFunction): Promise<void> => {
  try {
    res.json(await service.updateTaskStatus(req.params.id, getWeddingOwnerId(req), req.body.status));
  } catch (e) { next(e); }
};
